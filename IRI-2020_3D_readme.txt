...
batch processing:
  ...
  iri_ne     — Computes Ne profiles for a grid of lat/lon points
  iri_param  — Computes 2D ionospheric parameters (NmF2, hmF2, etc.)
...
-------------
  ...
  macOS:
    Option A (MacPorts):  sudo port install gcc14
                          Compiler: gfortran-mp-14
    Option B (Homebrew):  brew install gcc
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  Arguments:
    ...
    jchoice  0 = IRI computes all parameters internally (standard IRI)
             1 = Use parameters from param.txt (external NmF2, hmF2, etc.)
...
One altitude per line in kilometres, no header. Example (69 levels, 5-2250 km):
...
Benchmarked on a full tomoscand analysis run (global 1-degree grid):
  ...
  Before optimization:    user 597s,  elapsed 615s
  After optimization:     user 467s,  elapsed 177s  (3.5x wall-clock speedup)
...
IRI-2020 model:           irimodel.org
Original IRI readme:      00readme.txt in this directory
IRI output parameters:    irimodel.org/IRI-output-arrays.docs
IRI switches/options:     irimodel.org/IRI-Switches-options.docs
IRI FAQ:                  irimodel.org/docs/IRI_FAQ.pdf
